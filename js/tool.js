...
$(document).ready(function(){
   ...
   /* var $validator = $('#empresas-form-id').validate({
		  rules: {
		    firstname: {
		      required: true,
		      minlength: 3
    ...
    $('.wizard-card').bootstrapWizard({
        ...
        onNext: function(tab, navigation, index) {
            ...
            if(index==1){
               // $("#empresa-inscripcion-form-id").submit();
                 var curStep = $("#empresa-inscripcion-form-id"),
		  curInputs = curStep.find("input[type='text'],input[type='url'],textarea[textarea],select"),
                  $valid = true;
                  
                    for(var i=0; i<curInputs.length; i++){
                         if (curInputs[i].value==="" && $("#empresas-telefono").val()!="" && $("#empresas-telefono_extension").val()!=""){
                               $valid = false;
                               var a=curInputs[i].id;
                               var str=a.split('-');
                               $(curInputs[i]).closest(".form-group").addClass("has-error");
                               $(curInputs[i]).parent().find(".help-block").text(str[1]+" no puede estar vacío.");           
                         }
                         else
                         {
                               $(curInputs[i]).closest(".form-group").removeClass("has-error");
                               $(curInputs[i]).closest(".form-group").addClass("has-success");
                               $(curInputs[i]).parent().find(".help-block").text("");
                         }
                   }
                   
                   if (!$valid || curStep.find('.has-error').length) {
                       $.alert({
                            icon: 'glyphicon glyphicon-info-sign',
                            title: 'Advertencia',
                            theme: 'material',
                            content: 'Error debe llenar todos los campos en rojo',
            ...
            {
               // $("#empresa-inscripcion-form-id").submit();
                 var curStep = $("#contacto-inscripcion-form-id"),
		  curInputs = curStep.find("input[type='text'],input[type='url'],textarea[textarea],select"),
                  $valid = true;
            ...
            console.log("idd",tab, navigation, index);
        ...
        onTabShow: function(tab, navigation, index) {
            ...
            if($current >= $total) {
                ...
                $($wizard).find('.btn-exit').show();
            } else {
                $($wizard).find('.btn-next').show();
                $($wizard).find('.btn-finish').hide();
                $($wizard).find('.btn-exit').hide();
            }
