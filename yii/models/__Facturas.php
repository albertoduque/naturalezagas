...
class Facturas extends \yii\db\ActiveRecord
{
    ...
    public $serie,$direccion,$telefonoContacto,$tipoidentificacion
		...
		,$pais
		,$porcentajeIva,$clientes,$relacion_nc,$serie_nc,$id_nc,$identificacion, $verificacion,$facturaNC,$facturas,$identificacionFormat,$tipoDocumento			,$cantidadLineas			;
    ...
    {
        return [
            //[['numero', 'id_estado_factura', 'id_moneda','fecha','tipo_compra','periodo_pago', 'fecha_vencimiento','id_impuesto'], 'required'],
            [['numero', 'id_estado_factura', 'id_moneda','fecha'], 'required'],
            [[ 'id_estado_factura', 'id_moneda', 'id_empresa', 'id_persona', 'descuento','is_patrocinios', 'deleted','id_evento','id_serie','tipo_compra','id_impuesto'], 'integer'],
            ...
            [['created_at','send_xml','cufe', 'modified_at','fecha_transmision','direccion','serie_nc','id_nc','tipoDocumento','relacion_nc','telefonoContacto','tipoidentificacion'
				...
				,'ciudad','ciudadNombre'
				,'departamento','departamentoNombre'
				,'porcentajeIva','subtotal', 'iva', 'total','fecha','is_patrocinios','clientes'
				,'identificacion', 'verificacion','facturaNC','facturas','serie','respuesta','id_serie','trm','fecha_vencimiento','orden_compra','numeroaceptacioninterno','fechaemisionordencompra','cantidadLineas'], 'safe'],
            ...
            [['id_moneda'], 'exist', 'skipOnError' => true, 'targetClass' => Monedas::className(), 'targetAttribute' => ['id_moneda' => 'id']],			            [['id_impuesto'], 'exist', 'skipOnError' => true, 'targetClass' => Impuestos::className(), 'targetAttribute' => ['id_impuesto' => 'id']],			            [['id_medio_pago'], 'exist', 'skipOnError' => true, 'targetClass' => MedioPago::className(), 'targetAttribute' => ['id_medio_pago' => 'id']],
        ];
    }
	...
	/**     * @return \yii\db\ActiveQuery     */    public function getIdImpuesto()    {        return $this->hasOne(Impuestos::className(), ['id' => 'id_impuesto']);    }
	/**     * @return \yii\db\ActiveQuery     */    public function getIdMedioPago()    {        return $this->hasOne(MedioPago::className(), ['id' => 'id_medio_pago']);    }
