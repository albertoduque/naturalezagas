    ...
    public function copyEvent($idEvent=106,$eventos){
       // $empresas= Empresas::find()->where(['=','id_evento',$idEvent])->all();
        ...
        $empresas = (new \yii\db\Query())
            ->select("e.id as id_empresas,e.nombre as enombre,e.identificacion as eidentificacion,e.identificacion as eidentificacion"
        ...
        {
            if($empresa['id_empresas'] !== $empresa_id) {
