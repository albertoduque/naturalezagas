                    ...
                    try{
                        ...
                        if($model->validate())
                        {
                            ...
                            try {
								...
								$modelDepto = $this->findModelDepartamento("", $model->departamento);
								$model->departamentoNombre = $modelDepto->nombre;
                                $model->identificacionFormat = $this->calculaDigitoVerificador($model->identificacion);
                                ...
                                $xmlInvoice = $facturaWsdl->createFactura('e2556e2f2dc65b60653fab4fc380996647363a01', $params); //Desarrollo
								//var_dump($xmlInvoice);
								//var_dump(9);print "<br>";
                                $response = $objClientDispapelesApis->enviarFactura($xmlInvoice);
                    ...
                    try{
                        ...
                        if($model->validate())
                        {
                            ...
                            foreach ($detalle_factura as $df) {
								...
								$nombre_producto = $modelProductos->nombre;
								$impuesto_producto = $modelProductos->tipo_impuesto;
								$detalle_factura[0]["tipo_codigo_producto"] =  $codigo_producto;
								$detalle_factura[0]["nombre_producto"] =  $nombre_producto;
								$detalle_factura[0]["tipo_impuesto"] = $impuesto_producto;
...
//        var_dump(\yii\helpers\ArrayHelper::map($models, 'id', 'id_empresa'));
    ...
    public static function actionDropdownListaPatrocinio($id=0){
        ...
        $sql = "SELECT CONCAT('p-',id) as id,SUBSTRING(pro.nombre,1,100) as nombre "
                . "FROM `productos` pro WHERE  activo = 'S' AND inscripciones='N' AND id <>8 AND id not in ('$producto') AND id_evento=".$evento;
    ...
    protected function findModelInformacionEmpresa()
    {
